...
namespace TMPro.LearnWord
{
    ...
    public class TMP_TextSelector : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        public bool HasHoveredEverything => hoveredList != null && !hoveredList.Contains(false);
        ...
        public void IntializeHoveredList()
        {
            // Count Letters to interact with.
            m_TextComponent = gameObject.GetComponent<TMP_Text>();
            hoveredList = new bool[m_TextComponent.text.Length];
            for (int i = 0; i < m_TextComponent.text.Length; i++)
            {
                if (m_TextComponent.text[i] == ' ')
                {
                    hoveredList[i] = true;
                } else
                {
        ...
        void LateUpdate()
        {
            if (isHoveringObject)
            {
                ...
                /* //DESACTIVER PARCEQUE JE NE VEUT PAS ça LA
